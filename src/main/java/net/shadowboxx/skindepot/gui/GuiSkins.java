...
import net.shadowboxx.skindepot.upload.IUploadCompleteCallback;
import net.shadowboxx.skindepot.upload.ThreadMultipartPostUpload;
import net.shadowboxx.skindepot.upload.awt.IOpenFileCallback;
import net.shadowboxx.skindepot.upload.awt.ThreadOpenFilePNG;
...
//        this.screenTitle = manager;
    ...
    private void renderCubeMapTexture(float partialTick) {
        ...
        for (int blendPass = 0; blendPass < blendIterations * blendIterations; ++blendPass) {
            ...
            for (int cubeSide = 0; cubeSide < 6; ++cubeSide) {
                ...
                bb.pos(-1.0D, -1.0D, 1.0D).tex(0.0D, 0.0D).endVertex();
                bb.pos(1.0D, -1.0D, 1.0D).tex(1.0D, 0.0D).endVertex();
                bb.pos(1.0D, 1.0D, 1.0D).tex(1.0D, 1.0D).endVertex();
                bb.pos(-1.0D, 1.0D, 1.0D).tex(0.0D, 1.0D).endVertex();
                tessellator.draw();
                popMatrix();
            }
    ...
    private void rotateAndBlurCubemap() {
        ...
        for (int blurPass = 0; blurPass < blurPasses; ++blurPass) {
            ...
            bb.pos(this.width, this.height, this.zLevel).tex(0.0F + var7, 0.0D).endVertex();
            bb.pos(this.width, 0.0D, this.zLevel).tex(1.0F + var7, 0.0D).endVertex();
            bb.pos(0.0D, 0.0D, this.zLevel).tex(1.0F + var7, 1.0D).endVertex();
            bb.pos(0.0D, this.height, this.zLevel).tex(0.0F + var7, 1.0D).endVertex();
        }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTick) {
        ...
        this.drawCenteredString(this.fontRenderer, this.screenTitle, this.width / 2, 10, 0xffffff);

        this.fontRenderer.drawStringWithShadow(localSkin, 34, 34, 0xffffff);
        this.fontRenderer.drawStringWithShadow(serverSkin, this.width / 2 + 34, 34, 0xffffff);
        ...
        if (this.uploadError != null) {
            ...
            this.drawCenteredString(this.fontRenderer, failed, this.width / 2, this.height / 2 - 10, 0xFFFFFF55);
